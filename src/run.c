	...
	switch(opcode) {
		...
		case OPCODE_IMPORT_AS:
		{
			...
			if(name) {
				...
				if(!dict_cinsert(state, dest, name, state->segments[imported_segment].global_variables_map)) {
				...
				if(!dict_import(state, dest, state->segments[imported_segment].global_variables_map)) {
		...
		case OPCODE_BUILD_ARRAY:
		{
			...
			for(int i = 0; i < count; i++)
				if(!array_cinsert(state, object, i, object_pop(&state->eval_stack))) {
		...
		case OPCODE_PUSH_VARIABLE:
		{
			...
			if(object_stack_size(&state->vars_stack) > 0)
				object = dict_cselect(state, object_top(&state->vars_stack), variable_name);

			if(object == 0)
				object = dict_cselect(state, state->segments[u32_top(&state->segment_stack)].global_variables_map, variable_name);
		...
		case OPCODE_ASSIGN:
		{
			...
			if(!dict_cinsert(state, dest, variable_name, object_top(&state->eval_stack))) {
